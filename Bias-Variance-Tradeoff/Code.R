...
#the true data generating function is a 5-th order polynomial and we add Gaussian noise to it
...
for (i in 1:100){
  ...
  p <- p + 
    stat_smooth(method="lm", se=FALSE, fill=NA, formula=y~poly(x,3, raw=TRUE), aes_string(x, y), size = 0.1, color="blue") 
  
  
  q <- q + 
    stat_smooth(method="lm", se=FALSE, fill=NA, formula=y~poly(x,15, raw=TRUE), aes_string(x, y), size = 0.1, color="blue") 
  
}

print(p)
print(q)
df_MSE <- as.data.frame(cbind(train.3_MSE, test.3_MSE, train.15_MSE, test.15_MSE))
print(df_MSE)

ggplot()+
  geom_line(aes(x=1:100, y=df_MSE$train.3_MSE), color="black")+
  geom_line(aes(x=1:100, y=df_MSE$train.15_MSE), color="red" )+
  geom_line(aes(x=1:100, y=df_MSE$test.3_MSE),color="blue")+
  geom_line(aes(x=1:100, y=df_MSE$test.15_MSE), color="green", se=FALSE)
